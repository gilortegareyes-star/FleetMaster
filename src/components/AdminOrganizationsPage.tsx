...
import { ArrowLeft, Ban, Building2, CalendarDays, CheckCircle2, ChevronRight, Clock3, Copy, Edit3, Info, LockKeyhole, Mail, PauseCircle, PlayCircle, Plus, RefreshCw, Settings2, ShieldCheck, Trash2, UserPlus, UserRound, UserX, Users } from "lucide-react"
import { createOrganization, createOrganizationClientInvitation, deleteOrganization, disableOrganizationMembership, listOrganizationUsers, listOrganizations, OrganizationDeletionError, revokeOrganizationInvitation, sendManagerInvitation, setOrganizationOperationalAccess, setOrganizationStatus, updateOrganization } from "../services/organizations"
...
export function AdminOrganizationsPage({ onFeedback, onEnterOrganization, onRefreshSupportUnread, supportUnreadOrganizations, supportUnreadTicketIds }: { onFeedback: (message: string) => void; onEnterOrganization: () => void; onRefreshSupportUnread: () => Promise<void>; supportUnreadOrganizations: Array<{ organizationId: string; unreadCount: number }>; supportUnreadTicketIds: string[] }) {
  ...
  const handleStatusChange = async () => {
    ...
    setIsSaving(true)
    try { await setOrganizationStatus(selectedOrganization.id, nextStatus); await loadOrganizations(); setLifecycleAction(null); onFeedback(nextStatus === "suspended" ? "Empresa suspendida." : "Empresa reactivada.") }
  ...
  const openLifecycleModal = () => {
    if (selectedOrganization) setLifecycleAction(selectedOrganization.status === "active" ? "suspend" : "reactivate")
  }

  const closeLifecycleModal = () => {
    if (!isSaving) setLifecycleAction(null)
  }
  ...
  if (selectedOrganization) {
    ...
    {lifecycleAction ? <OrganizationLifecycleModal action={lifecycleAction} isSaving={isSaving} onClose={closeLifecycleModal} onConfirm={() => void handleStatusChange()} organization={selectedOrganization} /> : null}
   </section>
  }
...
function OrganizationLifecycleModal({ action, organization, isSaving, onClose, onConfirm }: { action: "suspend" | "reactivate"; organization: OrganizationSummary; isSaving: boolean; onClose: () => void; onConfirm: () => void }) {
  useEffect(() => {
    if (isSaving) return
  ...
  return <div className="organization-lifecycle-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose() }}><section aria-describedby="organization-lifecycle-description" aria-labelledby="organization-lifecycle-title" aria-modal="true" className="organization-lifecycle-modal" role="dialog"><header><div><p>{isSuspending ? "Suspender empresa" : "Reactivar empresa"}</p><h2 id="organization-lifecycle-title">{isSuspending ? `Suspender ${organization.name}` : `Reactivar ${organization.name}`}</h2></div><button aria-label="Cerrar" className="icon-button" disabled={isSaving} onClick={onClose} type="button">×</button></header><div className="organization-lifecycle-modal__body"><p id="organization-lifecycle-description">{isSuspending ? "Los usuarios de esta empresa perderán el acceso operativo a FleetMaster." : "La empresa volverá a estar activa."}</p><p>{isSuspending ? "La información de la empresa se conservará y podrás reactivarla posteriormente." : "Por seguridad, el acceso de sus usuarios permanecerá bloqueado hasta que lo liberes explícitamente."}</p></div><footer><button className="button button--secondary" disabled={isSaving} onClick={onClose} type="button">Cancelar</button><button className="button button--primary" disabled={isSaving} onClick={onConfirm} type="button">{isSaving ? "Guardando..." : isSuspending ? "Suspender empresa" : "Reactivar empresa"}</button></footer></section></div>
}
...
function OrganizationUsers({ organization, isSaving, onFeedback, onRefreshOrganizations, onSavingChange }: { organization: OrganizationSummary; isSaving: boolean; onFeedback: (message: string) => void; onRefreshOrganizations: () => Promise<void>; onSavingChange: (value: boolean) => void }) {
  const [users, setUsers] = useState<OrganizationUserRecord[]>([]); const [isLoading, setIsLoading] = useState(true); const [error, setError] = useState<string | null>(null); const [isInviteOpen, setIsInviteOpen] = useState(false); const [isClientInviteOpen, setIsClientInviteOpen] = useState(false); const [copiedInvitationId, setCopiedInvitationId] = useState<string | null>(null)
...
function ClientInvitationForm({ isSaving, onClose, onSubmit }: { isSaving: boolean; onClose: () => void; onSubmit: (name: string, email: string) => Promise<void> }) { const [name, setName] = useState(""); const [email, setEmail] = useState(""); const [error, setError] = useState<string | null>(null); const handleSubmit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setError(null); try { await onSubmit(name.trim(), email.trim()) } catch (submitError) { setError(submitError instanceof Error ? submitError.message : "No se pudo registrar la invitación.") } }; return <div className="modal-backdrop"><section aria-modal="true" className="organization-form-panel" role="dialog"><header><div><p>Nuevo usuario</p><h2>Agregar usuario</h2></div><button className="icon-button" onClick={onClose} type="button" aria-label="Cerrar">×</button></header><form onSubmit={handleSubmit}><label className="field"><span>Nombre completo</span><input autoFocus onChange={(event) => setName(event.target.value)} required value={name} /></label><label className="field"><span>Correo electrónico</span><input onChange={(event) => setEmail(event.target.value)} required type="email" value={email} /></label><p className="form-helper">Se registrará una invitación y se reservará una plaza para este usuario.</p>{error ? <p className="organization-form-error" role="alert">{error}</p> : null}<footer><button className="button button--secondary" onClick={onClose} type="button">Cancelar</button><button className="button button--primary" disabled={isSaving} type="submit"><UserPlus aria-hidden="true" size={17} /> {isSaving ? "Registrando..." : "Agregar usuario"}</button></footer></form></section></div> }
