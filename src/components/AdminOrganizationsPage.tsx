...
import { ArrowLeft, Ban, Building2, CalendarDays, CheckCircle2, Clock3, Edit3, Info, LockKeyhole, Mail, PauseCircle, PlayCircle, Plus, RefreshCw, Settings2, ShieldCheck, UserPlus, UserRound, UserX, Users } from "lucide-react"
  ...
  if (selectedOrganization) {
    ...
    return <section className="admin-page admin-detail-page">
      ...
      <header className="company-hero">
        ...
        <div className="company-hero__actions"><span className={`organization-status organization-status--${selectedOrganization.status}`}><CheckCircle2 aria-hidden="true" size={15} /> {selectedOrganization.status === "active" ? "Activa" : "Suspendida"}</span><button className="button button--primary" disabled={selectedOrganization.status !== "active"} onClick={() => { setActiveOrganization({ id: selectedOrganization.id, name: selectedOrganization.name, status: selectedOrganization.status }); onEnterOrganization() }} type="button"><Settings2 aria-hidden="true" size={17} /> Administrar empresa</button><button className="button button--secondary" onClick={openEditForm} type="button"><Edit3 aria-hidden="true" size={17} /> Editar</button></div>
      </header>
      ...
      </> : activeTab === "users" ? <OrganizationUsers organization={selectedOrganization} isSaving={isSaving} onFeedback={onFeedback} onRefreshOrganizations={loadOrganizations} onSavingChange={setIsSaving} /> : <FeedbackAdminPanel organizationId={selectedOrganization.id} />}
